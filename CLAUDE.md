...
`oyez_url` is normally a single URL string, but for a case consolidated from multiple Oyez case pages (e.g. `1971-176`) it's an array of URL strings instead — both forms are handled by `explorer.js` and `import_oyez.js`.

`number` holds one or more docket numbers joined by `;` (a joint case filed under several dockets, e.g. `"24-1260;24-1261"`). A `,` is **never** a delimiter — it's a literal character within a single docket number (e.g. a thousands separator in WA Supreme Court numbers like `"200,262-5"`). Split it with `splitDockets()`, take the canonical/primary docket with `primaryDocket()`, and render it for humans with `formatDockets()` (joins with `", "`) — all three are exported from `scripts/schema.js` and mirrored by hand in `assets/js/explorer.js` (and small copies in `terms.js` / `collections/*.js`).

`argument_consolidation` (optional) is a `;`-separated docket number(s) (same convention as `number`) of every case (this one included) — each a separately tracked case object (own `id`/`title`/`decision`) — heard in the same argument session, distinct from a joint `number` (one case filed under several dockets). Every case in the group carries the exact same value.
  ...
  "events": [
    {
      ...
      "audio_url": "https://…/mp3",
      "offset": 0,
      "transcript_url": "https://…/pdf",
      "text_file": "YYYY-MM-DD.json",
...
- `feeds/terms/YYYY-MM.json` — every audio-bearing event for one term (oldest first): `{ guid, title, date, type, case, caseId, link, audio_url, size, duration, durationSecs, source, advocates?, description }`.
...
- `FEED_IMAGE_URL` in `update_cases.js` points at `/assets/img/podcast-cover.jpg` — a 1909×1909 square crop of `assets/img/aa/ussc_exterior.jpg` (meets Apple/Spotify's 1400×1400 minimum).

### Sitemap (`courts/sitemap.xml`)
Generated by `node scripts/update_cases.js --sitemap` — never edit by hand. Every case/term/collection view in `explorer.js` is really the same static HTML shell (`courts/ussc/index.html`) with content resolved client-side from URL query params, so this is the only way search engines can discover the ~29,000 individual case URLs. Lives at `courts/` (not `courts/ussc/`) so it can cover every court; the entries are currently all ussc.
...
| `import_curiae.js` | Add `"Curiae: <work>"` tags to cases from a Yale curiae.law.yale.edu ranking page (Wayback URL or saved HTML); one tag per citing reference work | `node scripts/import_curiae.js <url-or-file> [...] [--dry-run] [--save DIR]` |
...
| `update_cases.js --sitemap` | Rebuild `courts/sitemap.xml` (one `<url>` per case/term/collection/topic) | `node scripts/update_cases.js --sitemap` |
...
## Notes regarding robots.txt

Every page using the "pane" layout (_layouts/pane.html) only makes sense wrapped in the real SPA's doc-viewer iframe at /courts/ussc/?link=<path> — nothing anywhere in this site ever links to one directly, and pane.html itself now JS-redirects a standalone visit there. Their bare URLs carry a static canonical tag that can't reflect the SPA state they're shown under, so keep them out of the crawl entirely rather than rely on that canonical (or the JS redirect, which a crawler may not follow) to sort it out after the fact. Every one of these rules was checked against the actual directory contents so real data files (JSON/images/PDFs the SPA fetches directly, e.g. courts/ussc/collections/*.json, courts/ussc/terms/YYYY-MM/cases.json, courts/ussc/collections/historical/orig/'s case thumbnails) stay crawlable — pages sharing a directory with such files are blocked by exact path ("$", not a trailing wildcard); dedicated pane-only subtrees are blocked recursively.
