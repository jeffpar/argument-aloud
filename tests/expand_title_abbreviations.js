...
// high-confidence title changes and opCite syncs to cases.json.
import fs from 'fs';
...
function filteredScdbTokens(caseName) {
    return caseName.split(/\s+/).map(cleanWord).filter(c => c && !STOPWORDS.has(c));
}
...
function resolveRun(words, slots, prevAnchor, nextAnchor, scdbTokens) {
    let gapWords = null, confidence = null;
    ...
    if (prevAnchor && nextAnchor) {
        ...
        const anchor = prevAnchor || nextAnchor;
        if (anchor) {
            const hits = findAll(scdbTokens, anchor);
            if (hits.length === 1) {
                const k = hits[0];
                gapWords = prevAnchor ? scdbTokens.slice(k + 1, k + 1 + slots.length)
                    : scdbTokens.slice(Math.max(0, k - slots.length), k).reverse();
                confidence = 'medium';
            }
        }
    }
    ...
    for (let s = 0; s < slots.length; s++) {
        ...
        if (!/^[A-Z]+$/.test(cand) || cand.length < 3 || cand[0] !== words[slots[s]][0]) return null;
    }
...
for (const { term, casesPath, cases } of termsData) {
    ...
    for (const c of cases) {
        ...
        const scdbTokens = filteredScdbTokens(row.caseName);

        const oldTitle = c.title;
        const titleParts = String(c.title).split('|');
        let titleTouched = false;
        const newTitleParts = titleParts.map(part => {
            ...
            for (let ki = 0; ki < words.length - 1; ki++) {
                const pair = resolveStatePair(words[ki], words[ki + 1], scdbTokens);
                if (!pair) continue;
            ...
            for (let ki = 0; ki < words.length; ki++) {
                ...
                const known = resolveKnownAbbrev(words[ki], scdbTokens);
            ...
            while (wi < words.length) {
                ...
                let prevAnchor = null;
                for (let j = wi - 1; j >= 0; j--) { const a = anchorForms(words[j]); if (a) { prevAnchor = a; break; } }
                let nextAnchor = null;
                for (let j = end + 1; j < words.length; j++) { const a = anchorForms(words[j]); if (a) { nextAnchor = a; break; } }

                const r = resolveRun(words, slots, prevAnchor, nextAnchor, scdbTokens);
                if (r) {
            ...
            if (resolved.every(r => r === null)) return part; // nothing to change
