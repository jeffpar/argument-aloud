    ...
    if (prevAnchor && nextAnchor) {
        ...
        if (anchor) {
            ...
            if (hits.length === 1) {
                const k = hits[0];
                if (prevAnchor) {
                    gapWords = scdbTokens.slice(k + 1, k + 1 + slots.length);
                    gapAbbrev = scdbAbbrevFlags.slice(k + 1, k + 1 + slots.length);
                } else {
                    gapWords = scdbTokens.slice(Math.max(0, k - slots.length), k).reverse();
                    gapAbbrev = scdbAbbrevFlags.slice(Math.max(0, k - slots.length), k).reverse();
                }
                confidence = 'medium';
            }
        }
...
for (const { term, casesPath, cases } of termsData) {
    ...
    for (const c of cases) {
        ...
        const { tokens: scdbTokens, abbrevFlags: scdbAbbrevFlags } = filteredScdbTokens(row.caseName);
        ...
        const newTitleParts = titleParts.map(part => {
            ...
            while (wi < words.length) {
                ...
                const r = resolveRun(words, slots, prevAnchor, nextAnchor, scdbTokens, scdbAbbrevFlags);
                if (r) {
