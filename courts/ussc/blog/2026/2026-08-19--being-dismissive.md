...
And then there are cases like these, the "dregs of the DIGs", which were "dismissed as improvidently granted" without any action (ie, no arguments and no statements from any of the Justices):
...
It's worth mentioning that in at least one of those examples, *N.Y. State Parole Bd. v. Coralluzzo*, it was noted that Justices White and Rehnquist dissented from the dismissal, but since no further explanation was provided, the case is effectively of no value to us.

There are other sub-species of DIGs, such as cases where the dissent *strenuously* argues (unsuccessfully, obviously) that the case *should* have been DIG'ed.  For example:
...
That case underscores an important point about DIGs: there is *always* an underlying reason for a DIG, but for Court to articulate that reason only when it wants to (eg, to admonish the parties), and to otherwise avoid admitting any mistakes of its own or simply help the parties understand what criteria they failed to satisfy, seems rather self-serving.
